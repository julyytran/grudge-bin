...
const GrudgesListItem = ({id, name, offense, forgiven, active}) => {
  if (active === true) {
    var selectButton = <button onClick={() => store.deselect(id)}>Deselect</button>
  } else {
    var selectButton = <button onClick={() => store.select(id)}>Select</button>
  }

  if (forgiven === "Forgiven? NOPE") {
    var forgiveButton = <button onClick={() => store.forgive()}>Forgive</button>
  } else {
    var forgiveButton = <button onClick={() => store.unforgive()}>Unforgive</button>
  }
